...
- Patched to COPY the project source into /app so manage.py exists at runtime and to auto-detect manage.py in the entrypoint.

Build
-----
    docker build -f .devcontainer/.docker-image-builder-archive/__owner__dtuait__project__api-security-ait-dtu-dk__image__app-main__tag__python-3.12-bookworm-django-5.1.1-postgres-16-04-10-2025/Dockerfile \
        -t app-main:python-3.12-django-5.1-postgres-16-patched .
...
4. Healthcheck probes `http://127.0.0.1:8121/healthz/` to match the project route.
5. Entrypoint improvements:
   - Auto-discovers `manage.py` under `/app/app-main`, `/app/app_main`, or `/app` if `APP_DIR` is not correct.
   - Optionally seeds an admin user when `DJANGO_ADMIN_USERNAME` and `DJANGO_ADMIN_PASSWORD` are provided.
