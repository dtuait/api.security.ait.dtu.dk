...
class AccessControlMiddleware(MiddlewareMixin):
    ...
    def __call__(self, request):
        ...
        print(f'running middleware for path: {path} and method: {method} and from user {request.user.username} and token_key: {token_key}')
        ...
        # if the user is not authenticated and the token is None, then redirect to login
        ...
        if not request.user.username.startswith('adm-') and not token_user:
