...
class UserProfile(models.Model):
    ...
    # The related_name 'members' is used to link UserProfiles to UserGroups
    # This relationship allows us to query a user's groups easily
    
    def has_endpoint_access(self, path, method=None):
        """
        Checks if the user has access to a specified endpoint through any of their groups.
        Access is determined by examining all EndpointPermissions related to the user's groups.
        """
        # Fetch all groups the user is part of
        groups = self.groups.all()
        
        # Query for endpoints matching the specified path (and method if specified)
        endpoint_qs = Endpoint.objects.filter(path=path)
        if method:
            endpoint_qs = endpoint_qs.filter(method=method)
        
        # Check if any of the user's groups have permission to access the endpoint
    ...
    def __str__(self):
        # Return the username associated with this UserProfile
...
class AccessRequest(models.Model):
    user_group = models.ForeignKey(UserGroup, on_delete=models.CASCADE, null=True)  # This replaces user_profile
    ...
    def __str__(self):
        user_group_name = self.user_group.name if self.user_group else 'Unknown User Group'
        return f"{user_group_name}'s access to {self.endpoint.path} is {self.get_status_display()}"
...
class EndpointPermission(BaseModel):
    ...
    def __str__(self):
        ...
        return f"{user_group_name} {access_status} {self.endpoint.path}"
