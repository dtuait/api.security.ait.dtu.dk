...
class BaseView(View):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Dynamic retrieval of limiter types associated with the user via AD groups
        from .models import LimiterType, IPLimiter, ADOrganizationalUnitLimiter
        user_limiter_types = []
        for limiter_type in LimiterType.objects.all():
            # Check each type of limiter for associations with user's AD groups
            ip_limiters = IPLimiter.objects.filter(limiter_type=limiter_type, ad_groups__in=user_ad_groups)
            ou_limiters = ADOrganizationalUnitLimiter.objects.filter(limiter_type=limiter_type, ad_groups__in=user_ad_groups)

            if ip_limiters.exists() or ou_limiters.exists():
                user_limiter_types.append(limiter_type)
