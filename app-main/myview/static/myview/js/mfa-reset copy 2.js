...
const hasMfaMethods = userAuthenticationMethods.value.some(method => 
...
if (hasMfaMethods) {
    ...
    app.uiBinder.mfaResetBtn.prop('disabled', false);

    // Add an event listener to the mfaResetBtn
    app.uiBinder.mfaResetBtn.on('click', async function (event) {
        event.preventDefault();
        try {
            // await app.appUtils.resetMFA(app);
            alert('MFA Reset button clicked');
        } catch (error) {
    ...
    // Display message that user has no MFA methods
    app.baseUIBinder.displayNotification('User has no MFA methods', 'alert-warning');
}
...
class UIBinder {
    constructor() {
        ...
        if (!UIBinder.instance) {
            ...
            this.mfaResetBtn = $('#mfa-reset-btn');
