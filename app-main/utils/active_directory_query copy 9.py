...
def active_directory_query(*, base_dn, search_filter, search_attributes=ALL_ATTRIBUTES, limit=None):
    try:
        ...
        excluded_attributes_to_process = []
        if search_attributes is ALL_ATTRIBUTES:
            excluded_attributes_to_process = ['objectGUID', 'objectSid', 'thumbnailPhoto', 'userCertificate', 'userParameters', 'distinguishedName', 'objectClass', 'msExchRecipientSoftDeletedStatus', 'sAMAccountName', 'msExchModerationFlags', 'msExchAddressBookFlags', 'primaryGroupID', 'garbageCollPeriod', 'msExchUserAccountControl', 'msExchMailboxAuditEnable', 'codePage', 'pwdLastSet', 'objectCategory', 'name', 'msExchUserBL', 'whenCreated', 'cn', 'countryCode', 'objectSid', 'msExchMDBRulesQuota', 'userAccountControl', 'sAMAccountType', 'instanceType', 'msExchMailboxAuditLogAgeLimit', 'accountExpires', 'dSCorePropagationData', 'msExchRecipientDisplayType', 'internetEncoding', 'msExchRecipientTypeDetails', 'msExchVersion', 'msExchProvisioningFlags', 'msExchTransportRecipient']
        ...
        while True:
            ...
            for entry in conn.entries:
                ...
                # Create a dictionary to hold attribute values for the current entry
                attr_dict = {}
                # Determine which attributes to process based on the search_attributes parameter
                attributes_to_process = entry.entry_attributes_as_dict.keys() if search_attributes is ALL_ATTRIBUTES else search_attributes
                for attr in attributes_to_process:
                    attr_values = entry.entry_attributes_as_dict.get(attr, [])
                    # Ensure every attribute value is presented as a list for consistency
