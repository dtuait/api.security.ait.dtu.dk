...
def active_directory_query(*, base_dn, search_filter, search_attributes=ALL_ATTRIBUTES, limit=None):
    try:
        ...
        excluded_attributes_to_process = ['thumbnailPhoto']
        ...
        while True:
            ...
            for entry in conn.entries:
                ...
                attr_dict = {}
                # attributes_to_process = [attr for attr in entry.entry_attributes_as_dict.keys() if attr not in excluded_attributes_to_process] if search_attributes is ALL_ATTRIBUTES else search_attributes
                # If search_attributes is set to ALL_ATTRIBUTES, process all attributes except those in excluded_attributes_to_process
                if search_attributes is ALL_ATTRIBUTES:
                    attributes_to_process = []
                    for attr in entry.entry_attributes_as_dict.keys():
                        # Only add the attribute to attributes_to_process if it's not in excluded_attributes_to_process
                        if attr not in excluded_attributes_to_process:
